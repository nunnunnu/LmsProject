...
public class ScoreAvgTotalControllaer {
    ...
    //과목별 전체 평균 조회
    @Operation(summary = "과목별 전체 평균 조회", description = "가장 최신 테스트 순으로 과목별 평균을 조회합니다.")
    @GetMapping("/avg")
    ...
    //과목별 상위 10% 평균 조회
    @Operation(summary = "과목별 상위 10% 평균 조회", description = "가장 최신 테스트 순으로 과목별 상위 10%학생들의 평균을 조회합니다.")
    @GetMapping("/top10")
    @Secured("ROLE_TEACHER")
